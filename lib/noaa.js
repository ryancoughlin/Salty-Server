...
export function tideTable(stationId) {
...
export function fetchWaterTemperature(stationId) {
  const waterTempParams = waterTemperatureQueryParams(stationId);
  const waterTempUrl = urlForWaterTemprature(stationId);

  return fetch(waterTempUrl)
    .then(res => res.json())
    .then(json => {
      console.log(json.data);
      return _.map(json.data, function(prediction) {
        return {
          time: moment(prediction.t),
...
function tideChartQueryParameters(stationId) {
  return {
    station: stationId,
    date: "today",
    range: "24",
...
function waterTemperatureQueryParams(stationId) {
  return {
    station: stationId,
    date: "today",
    range: "24",
...
function urlForWaterTemprature(stationId) {
  const queryParameters = waterTemperatureQueryParams(stationId);
...
function urlForTideChartQuery(stationId) {
  const queryParameters = tideChartQueryParameters(stationId);
  const encodedData = encodeData(queryParameters);
...
function urlForTideQuery(stationId) {
  ...
  const queryParameters = tidePredictionsQueryParameters(
    stationId,
    yesterday,
    future
  );
